...
namespace NCL {
	...
	protected:
		...
		bool		buttons[(int)MouseButtons::MAXSIZE];
		//Current button held state for each button
		bool		holdButtons[(int)MouseButtons::MAXSIZE];
		//Current doubleClick counter for each button
		bool		doubleClicks[(int)MouseButtons::MAXSIZE];
		//Counter to remember when last mouse click occured
		float		lastClickTime[(int)MouseButtons::MAXSIZE];
